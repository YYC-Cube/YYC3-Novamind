      ...
      <div className="pt-20 pb-8 px-6 h-screen flex">
            ...
            <div className="space-y-3">
              {slides.map((slide, index) => (
                <div
                  ...
                  className={`relative group cursor-pointer rounded-lg overflow-hidden transition-all duration-300 ${index === currentSlide ? "ring-2 ring-purple-400 scale-105" : "hover:scale-102"
                    }`}
                >
        ...
        <div className="flex-1 flex items-center justify-center">
          {slides.length > 0 && (
            <div
              ref={slideRef}
              className={`relative rounded-2xl shadow-2xl overflow-hidden transition-all duration-500 ${isPreviewMode ? "w-full h-full" : "w-4/5 aspect-video"
                }`}
              ...
              <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2">
                {slides.map((_, index) => (
                  <div
                    key={index}
                    onClick={() => setCurrentSlide(index)}
                    className={`w-3 h-3 rounded-full cursor-pointer transition-all duration-300 ${index === currentSlide ? "bg-white scale-125" : "bg-white/50 hover:bg-white/80"
                      }`}
                  />
                ))}
              </div>
