...
import { ArrowLeft, Award, BookOpen, Brain, MessageCircle } from "lucide-react"
...
export default function ShareContentPage() {
  ...
  const contentTypes: Array<{
    value: "question" | "answer" | "conversation" | "mindmap" | "learning_path"
    label: string
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const sharedContent = collaborationManager.shareContent({
        title: formData.title,
        description: formData.content,
        type: SHARE_TYPE_MAP[formData.type],
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {contentTypes.map((type) => (
                <label
                  key={type.value}
                  className={`flex items-start gap-3 p-4 border-2 rounded-lg cursor-pointer transition-all ${formData.type === type.value ? "border-primary bg-primary/5" : "border-gray-200 hover:border-primary"
                    }`}
                >
                  <input
                    type="radio"
