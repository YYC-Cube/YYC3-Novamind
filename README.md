...
**YYC³ NovaMind**（星图智语）是一套以「五维驱动」为架构理念的全栈 AI 智能交互平台：前端 Next.js 16 + React 19 + shadcn/ui，核心 Vercel AI SDK v5 驱动，覆盖 **对话 · 生成 · 学习 · 社区** 四大场景，具备从用户请求到数据反馈的 **全链路生产闭环**。

### 核心能力一句话

- **对用户**：一个平台完成 AI 对话、思维导图/海报/网页/PPT 生成、学习路径规划、知识图谱探索与社区协作，支持 PWA 全端安装与离线使用
- **对开发者**：八资产工程闭环（防护护栏 → 持久化 → 认证 → 工具循环 → Provider 路由 → RAG → MCP 双向 → 反馈闭环），三道质量门禁（typecheck / lint 棘轮 / 覆盖率棘轮）+ OpenAPI 契约同源，fork 即可二次开发
- **对生态**：既是 MCP 消费方（可接入任意外部工具服务器），也是 MCP 供方（`POST /api/mcp` 把四大生成能力反向暴露给 Claude Desktop / Cursor 等 host）

### 设计原则

| 原则 | 落地方式 |
| :----- | :----- |
| **DSN/密钥驱动降级** | 数据库、Sentry、AI Key 任一缺失 → 对应能力静默关闭，应用零崩溃运行 |
| **契约即文档** | OpenAPI SSOT 由代码注解生成，Swagger UI + CI drift 门禁双端同源 |
| **棘轮只紧不松** | ESLint 豁免清单与覆盖率阈值只减不增，代码质量增量零容忍 |
| **生产护栏内建** | 所有小模型入口默认挂载限流/消毒/成本闸，多 Provider 链式故障转移 |
...
> 完整命令手册（含占位符说明、生产供给清单、一键校验脚本）：[docs/yyc3-novamind-m3-20260918/07-生产供给配置手册.md](./docs/yyc3-novamind-m3-20260918/07-生产供给配置手册.md)
